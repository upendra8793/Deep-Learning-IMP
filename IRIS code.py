...
def compare_images(filepath1, filepath2):
    print("compare images started " + filepath1)
    ...
    print("compare images end " )


def compare_binfiles(bin_path1, bin_path2):
    print("compare_binfiles - started " )
    ...
    print("compare_binfiles - end " )

def load_rois_from_image(filepath):
    print("load_rois_from_image - start " )
    ...
    print("load_rois_from_image - end " )
...
def get_iris_boundaries(img, show=False):
    print("get_iris_boundaries- start" )
    ...
    if show:
        ...
        ch = cv2.waitKey(0)
        cv2.destroyAllWindows()
    print("get_iris_boundaries- end")
    return pupil_circle, ext_iris_circle


def find_pupil(img):
    print("find_pupil- start")
    def get_edges(image):
    ...
    while (param2 > 35 and len(pupil_circles) < 100):
        for mdn, thrs in [(m, t)
          ...
          #  print("Edges ::", edges)
            ...
            circles = cv2.HoughCircles(edges, cv2.HOUGH_GRADIENT, 1, 1,np.array([[]]),
                                        param1, param2)
            #we got circles in the format 3D array
            print("Circles ::", circles)
            circles
            # print("first circel ::", circles)
    ...
    cimg = cv2.cvtColor(img, cv2.COLOR_GRAY2BGR)
    print("find pupil end")
    return get_mean_circle(pupil_circles)


def get_mean_circle(circles, draw=None):
    print("get_mean_circle start")

#    temp = [c[0] for c in circles]
    print("Debug - get_mean_circles checkpoint5")
    idx = 0
    #circles.remove(200)
    #circles.remove(200)
    #circles.remove(200)
...
def get_rois(img, pupil_circle, ext_circle, show=False):
    ...
    rois['right-side']['img'] = \
        rois['right-side']['img'][0:round(2.5) * ext_circle[2],0:round(1.25) * ext_circle[2]]
    rois['left-side']['img'] = \
        rois['left-side']['img'][0:round(2.5)* ext_circle[2], 0:round(1.25)* ext_circle[2]]
    rois['bottom']['img'] = \
        rois['bottom']['img'][0:round(1.25)* ext_circle[2], 0:round(2.5) * ext_circle[2]]
    rois['complete']['img'] = \
        rois['complete']['img'][0:round(2.5) * ext_circle[2], 0:round(2.5) * ext_circle[2]]
...
if __name__ == "__main__":
    ...
    filepath1 = r'F:/PYTHON PROJECTS IMPO/IRIS Face Recognition/IRIS_Main_project/Entries/prasad12.jpg'
    filepath2 = r'F:/PYTHON PROJECTS IMPO/IRIS Face Recognition/IRIS_Main_project/Entries/prgati2.jpg'
